...
namespace CtrTex.ETC1.Helper
{
    ...
    internal class Optimizer
    {
        ...
        private readonly int _maxRefinementTrials = 4;
        ...
        {
            _maxRefinementTrials = (Etc1Support.Quality == Etc1Quality.Low) ? 2 : 4;
            _pixels = pixels;
            _limit = limit;
        ...
        private bool EvaluateSolution(RGB c, int[] intenTable)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                for (int j = 0; j < _maxRefinementTrials; j++)
                {
                ...
                foreach (var diff in new[] { false, true }) // let's again just assume no diff
                {
                    ...
                    for (i = 0; i < 2; i++)
                    {
                        ...
                        if (i == 1 && diff)
                        {
                            ...
                            if (Etc1Support.Quality < Etc1Quality.High) { if (!opt.ComputeDeltas(-3, -2, 2)) break; }
                            else { if (!opt.ComputeDeltas(-4, -3, -2, -1, 0, 1, 2, 3)) break; }
                        }
                        else
                        {
                            if (Etc1Support.Quality >= Etc1Quality.High) { if (!opt.ComputeDeltas(-4, -3, -2, -1, 0, 1, 2, 3, 4)) break; }
                            else if (Etc1Support.Quality == Etc1Quality.Medium) { if (!opt.ComputeDeltas(-1, 0, 1)) break; }
                            else { if (!opt.ComputeDeltas(0)) break; }

                            // Fix fairly arbitrary/unrefined thresholds that control how far away to scan for potentially better solutions.
                            if (opt._bestSln.Error > 3000)
                            {
                                if (Etc1Support.Quality <= Etc1Quality.Medium)
                                {
                                    opt.ComputeDeltas(-3, -2, 2, 3);
                                }
                                else
                                {
                                    if (opt._bestSln.Error > 6000)
